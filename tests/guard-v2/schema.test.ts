...
// ---------------------------------------------------------------------------
// schema.parse — Result<T, GuardErr[]>
// ---------------------------------------------------------------------------
...
describe('custom error messages', () => {
	it('uses guard .err() messages in schema errors', () => {
		const schemas = defineSchemas({
			User: is.object({
				name: is.string.err('Name must be a string'),
				age: is.number.err('Age must be a number'),
			}),
		});
