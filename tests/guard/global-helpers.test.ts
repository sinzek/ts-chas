...
describe('Universal helpers (v2)', () => {
	...
	describe('.error()', () => {
		...
		it('sets a dynamic error message from function', () => {
			const guard = is.string.error(ctx => `${ctx.meta.name} is required`);
			expect(guard.parse(42).unwrapErr().message).toBe('string is required');

			const guard2 = is.string.error(ctx => `${ctx.value} is not a string`);
			expect(guard2.parse(42).unwrapErr().message).toBe('42 is not a string');
		});
	...
	describe('Error messages', () => {
		...
		it('dynamic .err() function receives meta', () => {
			const guard = is.number.positive.error(ctx => `Guard "${ctx.meta.name}" rejected the value`);
