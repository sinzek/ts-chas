...
import { ok, err, ResultAsync, okAsync, errAsync } from '../src/result.js';
import { defineErrs, type InferErr } from '../src/tagged-errs.js';
...
describe('Error Context', () => {
	describe('Result.context', () => {
		...
		it('should chain multiple contexts (most recent first)', () => {
			const result = err(TestErrs.NotFound('user'))
				.context('step 1')
				.context('step 2')
				.context('step 3');
		...
		it('should work in andThen chains', () => {
			...
			const step2 = (v: number) => err(TestErrs.Validation('email')).context('step 2');
